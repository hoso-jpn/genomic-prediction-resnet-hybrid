...
**追加の差分検査（同日、文書修正後）**: 上記の後、文書修正（重み・加工済み配列の断定の修正、ディスク記述の修正）とスタック伝播を行ったため、その差分だけを再度検査しました。対象head: `8a7381f` / `fc725de` / `35e4057` / `efaa15b`(#13) / `1296e55`(#14) / `7159e5b`(#15。この段落を追加する直前の版) / `13b5349`(#16)。範囲は**5 commit / 4 blob / 69,107 bytes**＋commit message＋更新後のPR本文（32,069 chars）。既存履歴は不変のため再検査していません。結果は、`detect-secrets` 0件、**認証情報クラスはblob・commit message・PR本文いずれも0件**、バイナリblob 0件。メールアドレスはblobで2種類（本文書に検査結果として記載したサービス用アドレス）、commit messageで2種類（gitのcommitter情報）、PR本文で2種類（同じサービス用アドレス）。`/home/<user>`形式のパターンはblobで1件一致しましたが、値は移設先候補として例示した`/home/containerd`であり、**利用者のログイン名を含む絶対パスではありません**（誤検出）。
