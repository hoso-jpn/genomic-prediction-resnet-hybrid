...
class GblupBaselineTest(unittest.TestCase):
    ...
    def test_raw_loader_excludes_parent_and_preserves_missing(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            phenotype.to_csv(
                data_dir / f"{family_id}{gblup.PHENOTYPE_SUFFIX}",
                sep="\t",
                index=False,
                compression="gzip",
            )
            genotype.to_csv(
                data_dir / f"{family_id}_4312{gblup.GENOTYPE_SUFFIX}",
                sep="\t",
                index=False,
                compression="gzip",
            )

            dataset = gblup.load_soynam_dataset(data_dir)
    ...
    def test_pairing_mismatch_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            pd.DataFrame(
                ...
                data_dir / f"A_NAM01{gblup.PHENOTYPE_SUFFIX}",
                sep="\t",
                index=False,
                compression="gzip",
            )
            with self.assertRaisesRegex(ValueError, "pairing is invalid"):
                gblup.load_soynam_dataset(data_dir)
