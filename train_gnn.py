...
def run_gnn_training(argv=None):
    ...
    print(
        f"データ読み込み完了 | 個体数: {len(y_all)} | SNP数: {X_all.shape[1]} | 遺伝子数: {num_genes} | device: {device}"
    )
    ...
    for fold, (train_idx, test_idx) in enumerate(
        logo.split(X_all, y_all, groups=family_ids)
    ):
        ...
        for _epoch in range(config["epochs"]):
            ...
            for i in range(0, X_train.shape[0], config["batch_size"]):
                ...
                N = batch_y.size(0)
                batch_snp_map = (
                    snp_to_gene_map_dev.repeat(N)
                    + torch.arange(N, device=device).repeat_interleave(X_train.shape[1])
                    * num_genes
                )
                batch_indices = torch.arange(N, device=device).repeat_interleave(
                    num_genes
                )
                optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            test_snp_map = (
                snp_to_gene_map_dev.repeat(N_test)
                + torch.arange(N_test, device=device).repeat_interleave(X_test.shape[1])
                * num_genes
            )
            test_batch_indices = torch.arange(N_test, device=device).repeat_interleave(
                num_genes
            )
            y_pred = (
                model(X_test_t, test_snp_map, edge_index_dev, test_batch_indices)
                .cpu()
                .numpy()
                .flatten()
            )
        ...
        if np.isnan(test_corr):
            print(
                f"Fold {fold + 1:2d} | Test correlation: nan  "
